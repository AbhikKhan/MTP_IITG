...
def generateGraph(d, inputFile, outputFile):
    graph = []
        ...
        for key in keys:
            if (key[0] == "w") & (node[key] != 0):
                if int(key[1]) - int(key[2]) > 1:
                    opfile.write(
                        f'\t{key[1]} -> {key[2]} [label = "{node[key]}", color = "red"];\n'
                    )
                elif int(key[1]) - int(key[2]) == 1:
                    opfile.write(
                        f'\t{key[1]} -> {key[2]} [label = "{node[key]}", color = "black"];\n'
                    )
            elif (key[0] == "x") & (node[key] != 0):
                if key[2] == "1":
                    opfile.write(
                        f'\t{key} -> {key[1]} [label = "{node[key]}", color = "blue"];\n'
                    )
                    opfile.write(f'\t{key} [label = "sample", shape = "box"];\n')
                else:
                    opfile.write(
                        f'\t{key} -> {key[1]} [label = "{node[key]}", color = "green"];\n'
                    )
